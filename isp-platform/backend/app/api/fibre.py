...
from ..models import (
    FibreRoute as FibreRouteModel, FibreCore as FibreCoreModel, MSTBox, Client, ActivityLog,
    FibreCoreStatus, get_fiber_color
)
from ..schemas import (
    FibreRoute as FibreRouteSchema, FibreRouteCreate, FibreRouteUpdate,
    FibreCore as FibreCoreSchema, FibreCoreCreate, FibreCoreUpdate
)
...
@router.get("/routes", response_model=List[FibreRouteSchema])
async def get_fibre_routes(
    ...
    result = await db.execute(
        select(FibreRouteModel).offset(skip).limit(limit).order_by(FibreRouteModel.created_at.desc())
    )
    return result.scalars().all()


@router.get("/routes/{route_id}", response_model=FibreRouteSchema)
async def get_fibre_route(
    ...
    result = await db.execute(
        select(FibreRouteModel).where(FibreRouteModel.id == route_id)
    )
...
@router.post("/routes", response_model=FibreRouteSchema, status_code=status.HTTP_201_CREATED)
...
@router.get("/routes/{route_id}/cores", response_model=List[FibreCoreSchema])
async def get_route_cores(
    ...
    query = select(FibreCoreModel).where(FibreCoreModel.route_id == route_id)

    if status_filter:
        query = query.where(FibreCoreModel.status == status_filter)

    result = await db.execute(query.order_by(FibreCoreModel.core_number))
    return result.scalars().all()


@router.get("/cores/{core_id}", response_model=FibreCoreSchema)
async def get_core(
    core_id: int,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(get_current_user)
    ...
    result = await db.execute(
        select(FibreCoreModel).where(FibreCoreModel.id == core_id)
    )
...
@router.put("/cores/{core_id}", response_model=FibreCoreSchema)
...
@router.post("/cores/{core_id}/assign-client/{client_id}", response_model=FibreCoreSchema)
...
async def get_fibre_stats(
    ...
    total_routes = await db.execute(select(func.count(FibreRouteModel.id)))
    total_cores = await db.execute(select(func.sum(FibreRouteModel.total_cores)))
    used_cores = await db.execute(
        select(func.count(FibreCoreModel.id)).where(FibreCoreModel.status == FibreCoreStatus.USED)
    )
    free_cores = await db.execute(
        select(func.count(FibreCoreModel.id)).where(FibreCoreModel.status == FibreCoreStatus.FREE)
    )
    faulty_cores = await db.execute(
        select(func.count(FibreCoreModel.id)).where(FibreCoreModel.status == FibreCoreStatus.FAULTY)
    )
    total_distance = await db.execute(select(func.sum(FibreRouteModel.distance_meters)))
